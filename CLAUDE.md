...
- **A shared development project accumulates other prototypes' data.** Expect leftover
  Product Types, attribute values and extensions from unrelated prototyping (e.g.
  `perf-cartsize-*` Product Types, `ext-lab-*` resources) if you're pointed at a shared
  commercetools project rather than one dedicated to this agent. Inventory what is there
  before repurposing anything, and expect catalogue oddities to be data, not bugs.
