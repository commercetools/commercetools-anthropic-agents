...
def test_a_configured_test_prefix_is_stripped_from_display_values(monkeypatch):
    """A project whose catalogue carries a leftover test-data tag (here "KMB", from some
    earlier test run) sets CT_TEST_VALUE_PREFIX; offering a shopper "KMB Gold" as a finish
    is worse than showing nothing."""
    monkeypatch.setenv("CT_TEST_VALUE_PREFIX", "KMB")
...
def test_no_test_prefix_configured_leaves_values_unchanged(monkeypatch):
    monkeypatch.delenv("CT_TEST_VALUE_PREFIX", raising=False)
    assert strip_test_prefix("KMB Gold") == "KMB Gold"
