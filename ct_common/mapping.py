...
# A display normalisation for projects whose catalogue carries a leftover test-data tag on
# some attribute values (e.g. an earlier test run's "KMB Gold", "KMB- Dry clean only"), so a
# shopper is offered a tagged value as a finish and the same colour appears twice, once
# tagged and once not. The catalogue data is not ours to rewrite, and the tag is not a
# merchandising fact, so it is stripped on the way out. Empty by default -- no stripping
# happens unless a project sets CT_TEST_VALUE_PREFIX to the tag its own catalogue carries.
# Read per call, not cached at import time, so a project (and a test) can set it at any point.
def _test_value_prefix() -> str:
    return os.environ.get("CT_TEST_VALUE_PREFIX", "")
...
def strip_test_prefix(value: str) -> str:
    """With ``CT_TEST_VALUE_PREFIX`` set, ``"KMB Gold"`` and ``"KMB- Dry clean only"``
    render without the tag; unset, every value passes through unchanged."""
    prefix = _test_value_prefix()
    if not prefix or not value.startswith(prefix):
        return value
    return value[len(prefix) :].lstrip("- ").strip() or value
